...
def getFileDialog(message_in, default_path=os.path.dirname(__file__)):
    app = wx.App(0)

    try:
        fileDialog =  wx.FileDialog(None, message_in, defaultDir=default_path, wildcard="CSV files (*.csv)|*.csv",
                       style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
    except IndexError:
        fileDialog =  wx.FileDialog(None, message_in, defaultDir=os.path.dirname(__file__), wildcard="CSV files (*.csv)|*.csv",
                       style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
...
class DirHandler:
    def __init__(self, name, dir=os.path.dirname(__file__)):
        self.name = name 
        self.cur_directory = dir
        self.dirs = {}

        self.log_name = f"{self.name}-log.log"

        try:
            with open(self.cur_directory + r'\\' + self.log_name, 'r+') as dirlog:
                pass
        except:
            with open(self.cur_directory + r'\\' + self.log_name, 'w+') as dirlog:
                pass
