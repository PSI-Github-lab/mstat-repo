...
Arguments:
    ...
    <lda_dim>               - (Integer) number of dimensions for LDA analysis (must be less than number of labels)
...
def main():
    ...
    if not argm:
        print("Type 'python cluster.py help' for more info")
        quit()
    else:
        file_name = argm[0]
    ...
    if file_name == 'rand':
        ...
        # together.
        feature_data, labels = make_blobs(n_samples=500,
                        n_features=2,
                        centers=4,
                        cluster_std=1,
        ...
        X = Normalizer().fit_transform(feature_data)
        X = PCA(n_components=200).fit_transform(X)
        X = LDA(n_components=14, solver='eigen').fit_transform(X, labels)

    

    range_n_clusters = [12,13,14,15,16,17,18]
