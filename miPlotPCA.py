...
class InteractivePlot:
    ...
    def __init__(self, training_file_reader, test_file_reader, pipeline, n_std) -> None:
        ...
        plt.suptitle('PCA Analysis of MS Data', color=title_color) 
        ...
        if test_frame is not None:
            ...
            self.plot_frame = pd.concat([training_frame, test_frame], ignore_index=True)
        else:
        ...
        _, self.loadings = self.initAxis2(None, title_color)
    ...
    def initAxis1(self, ax, training_frame, test_frame, title_color, n_std):
        ...
        ax.set_title('PCA Score Plot', color=title_color)
        ax.set_xlabel('PC 1', color=title_color)
        ax.set_ylabel('PC 2', color=title_color)
    ...
    def initAxis2(self, lda, title_color):
        ...
        loadings = pca_loadings#np.dot(lda_loadings, pca_loadings)
    ...
    def loadTrainTestData(training_file_reader, test_file_reader, pipeline):
        ...
        if test_file_reader is None:
            ...
            _, _, test_labels, _ = test_file_reader.encodeData()
    ...
    def projectDataFrames(self, training_file_reader, test_file_reader, lda_training_data, lda_test_data, test_predictions):
        if lda_test_data is None or test_file_reader is None:
            ...
            self.data_frame = pd.concat([training_file_reader.file_frame, test_file_reader.file_frame], sort=False, ignore_index=True)
        
        if lda_training_data.shape[1] > 1:
            training_frame = pd.DataFrame({
                'index' : list(range(1, len(training_file_reader.file_frame.iloc[:,0])+1)),
            ...
            if lda_test_data is None:
                ...
                test_frame = pd.DataFrame({
                    'index' : list(range(len(training_file_reader.file_frame.iloc[:,0])+1, len(test_file_reader.file_frame.iloc[:,0])+len(training_file_reader.file_frame.iloc[:,0])+1)),
                    'label' : 'unknown',
...
help_message = """
Console Command: python interactiveplot2D_mpl.py <model_data_name.csv> <test_data_name.csv> <n_pca_dimensions>
Arguments:
    ...
    <n_pca_dimensions>         - (Integer) Number of PCA dimensions """
...
def main():
    if argm := handleStartUpCommands(help_message):
        ...
        if len(argm) == 2:
            ...
            n_pca_dim = int(argm[1])
        else:
            second_csv_file = argm[1]
            n_pca_dim = int(argm[2])
    ...
    steps = [
            #('scl', MinMaxScaler()),
            ('dim', PCA(n_components=n_pca_dim, random_state=rnd_state)),
            ]
    pca = Pipeline(steps)
    pca.fit(training_file_reader.getTICNormalization())

    plot = InteractivePlot(training_file_reader, test_file_reader, pca, 0.0)
    plot.show()
