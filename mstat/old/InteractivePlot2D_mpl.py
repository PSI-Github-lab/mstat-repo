...
class InteractivePlot:
    ...
    def __init__(self, training_file_reader, test_file_reader, pipeline, n_std) -> None:
        ...
        if test_file_reader is None:
            ...
            self.label_names = np.concatenate((np.unique(training_labels), ['unknown']))
        ...
        if test_frame is not None:
            test_frame['color'] = len(self.label_names)
        ...
        self.scores_x, self.scores_y = self.initAxis1(self.ax1, ax1_color, n_std)
    ...
    def plot_means(self, ax):
        ...
        for mean in proj_means:
            try:
                ...
                self.ax1.scatter(mean[0], 0, c='black', marker='x', s=100, linewidths=3)

    def initAxis1(self, ax, title_color, n_std):
        label_names = self.label_names
        qda_classifier = self.qda_classifier
        colormap = self.colormap
        x = self.plot_frame['ld1']
        try:
            y = self.plot_frame['ld2']
            dim_flag = True
        except KeyError:
            y = np.zeros(x.shape)
            dim_flag = False
        self.sc = ax.scatter(x, y, color=self.colormap(self.plot_frame['color']))
...
help_message = """
Console Command: python interactiveplot2D_mpl.py <model_data_name.csv> <test_data_name.csv> <pcalda_model.model> <n_std>
Arguments:
