...
class InteractivePlot:
    ...
    def __init__(self, training_file_reader, test_file_reader, n_comp) -> None:
        # load data from training and test data sources
        training_data, test_data, training_labels, encoder = self.loadTrainTestData(training_file_reader, test_file_reader)

        self.pca = PCA(n_components=n_comp, random_state=5)
        self.pca.fit(training_data)

        model_training_data, model_test_data = self.pca.transform(training_data), self.pca.transform(test_data)

        # format the data into dataframes that are easier to plot and keep track of each point source
        training_frame, test_frame = self.projectDataFrames(training_file_reader, test_file_reader, model_training_data, model_test_data)
        ...
        self.initAxis1(ax1_color, self.colormap, label_names)

        # axis 2: Explained variance
        self.initAxis2(title_color)
    ...
    def plot_means(self):
        ...
        for mean in proj_means:
            self.ax1.scatter(mean[0], mean[1], c='black', marker='x', s=100, linewidths=3)

    def initAxis1(self, title_color, colormap, label_names):
        self.sc = self.ax1.scatter(self.plot_frame['ld1'], self.plot_frame['ld2'], color=self.colormap(self.plot_frame['color']))

        # means
        #self.plot_means()
        
        # label plot with axis names, title, et cetera
        self.ax1.set_title(f'PCA', color=title_color)
    ...
    def initAxis2(self, title_color):
        # plot the explained variance data 
        loadings = self.pca.components_
        
        x = np.linspace(1, loadings.shape[1], loadings.shape[1])
        ...
        #self.ax2.stem(x, -np.abs(loadings[1]), 'r')

        # label plot with axis names, title, et cetera
        #self.ax2.set_xticks(np.arange(min(x), max(x)+1, 1.0))
        #self.ax2.set_ylim([0,108])
        self.ax2.set_title('Loadings', color=title_color)
        self.ax2.set_xlabel('Bin', color=title_color)
    ...
    @staticmethod
    def loadTrainTestData(training_file_reader, test_file_reader):
        ...
        training_data = training_file_reader.getTICNormalization()
        test_data = test_file_reader.getTICNormalization()

        return training_data, test_data, training_labels, encoder
    ...
    def highlightPCALDA(self, event, ind):
        ...
        inten = np.nan_to_num(self.data_frame[self.data_frame['filename'] == self.point_label].values[0][4:], nan=0.0)
...
Arguments:
    ...
    <n_comp>                    - (Float) Number of standard deviations that the confidence ellipsoids should extend to"""
...
def main():
    ...
    plot = InteractivePlot(training_file_reader, test_file_reader, n_comp)
    plot.show()
